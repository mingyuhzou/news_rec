...
import os
import logging
from rec.transformer.data.tf_dataset import GenRecDataset, GenRecDataLoader
from config.tf import cfg
from rec.utils.metrics import ndcg_at_k,recall_at_k
from rec.transformer.model.T5 import T5
...
def main(cfg):
    ...
    train_dataset = GenRecDataset(
        dataset_path=os.path.join(cfg['dataset_path'], 'train/train.parquet'),
        code_path=cfg['code_path'],
        max_len=cfg['max_len'],
        code_len=cfg['code_len'],
    )
    test_dataset = GenRecDataset(
        dataset_path=os.path.join(cfg['dataset_path'],'test/test.parquet'),
        code_path=cfg['code_path'],
        max_len=cfg['max_len'],
        code_len=cfg['code_len'],
    )
