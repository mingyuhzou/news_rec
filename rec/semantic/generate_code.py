...
from config.generate_sid import cfg as model_cfg
from config.generate_code import cfg as process_cfg
from rec.semantic.model.RQ_VAE import RQVAE
from rec.semantic.model.kmeans import RQKmeans
from rec.utils.collision import check_collision, get_collision_item, get_indices_count
from rec.semantic.trainer.sid_trainer import train
...
def sid_from_VAE():
    ...
    all_indices_str = np.array(
        all_indices_str)  # ["['<a_226>', '<b_248>', '<c_1>']" "['<a_180>', '<b_11>', '<c_248>']""['<a_15>', '<b_219>', '<c_40>']" "['<a_63>', '<b_199>', '<c_90>']""['<a_202>', '<b_229>', '<c_243>']"]
...
def sid_from_KMEANS():
    ...
    # 3. 初始化 RQKmeans
    # 注意：当前 RQKmeans 只支持每层 codebook_size 一样
    assert len(set(model_cfg["num_emb_list"])) == 1

    model = RQKmeans(
        dim=model_cfg["in_dim"],
        codebook_size=model_cfg["num_emb_list"][0],
        n_layers=len(model_cfg["num_emb_list"]),
    ).to(device)
    ...
    for indices in all_indices.tolist():
        code = [
            int(item.split("_")[1].strip(">"))
            for item in indices
        ]
        codes.append(code)

    codes_array = np.array(codes)  # [N, L]

    # 7. 追加一列 conflict id，用来强制去重
    codes_array = np.hstack(
        (
            codes_array,
            np.zeros((codes_array.shape[0], 1), dtype=int),
        )
    )
    ...
    if len(duplicates) > 0:
        ...
        for duplicate in duplicates:
            duplicate_indices = np.where(
                (codes_array == duplicate).all(axis=1)
            )[0]

            for i, idx in enumerate(duplicate_indices):
                codes_array[idx, -1] = i

    new_unique_codes, new_counts = np.unique(
        codes_array,
        axis=0,
        return_counts=True,
    )
    ...
    codes_array_raw = codes_array[:, :-1]

    for i in range(codes_array_raw.shape[1]):
        usage = len(np.unique(codes_array_raw[:, i]))
        total = model_cfg["num_emb_list"][i]
        print(f"Layer {i} codebook usage: {usage}/{total}, rate={usage / total:.4f}")

    np.save(process_cfg["output_file"], codes_array)


if __name__ == "__main__":
    if model_cfg['sid_method'] == 'RQ-KMEANS':
        sid_from_KMEANS()
    elif model_cfg['sid_method'] == 'RQ-VAE':
        sid_from_VAE()
    else:
        raise Exception("Unknown sid_method")