...
class GenRecDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        for item in processed_data:
            ...
            item['target'] = self.item2code.get(item['target'], np.array([self.PAD_TOKEN] * 4))
...
class GenRecDataLoader(DataLoader):
    ...
    def collate_fn(self,batch,pad_token=0):
        ...
        flattened_targets=torch.stack(
            [torch.tensor(target,dtype=torch.int64) for target in target]
        )
        ...
        return {'history':flattened_histories,'target':flattened_targets,'attention_masks':attention_masks}